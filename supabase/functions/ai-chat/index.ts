...
serve(async (req) => {
  ...
  try {
    const { messages, model = 'google/gemini-2.5-flash', userId } = await req.json();
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: "system", 
            content: `You are an expert Solana mobile dApp architect. Help users build their apps by:
- Suggesting templates and features
- Providing code snippets and best practices
- Explaining Solana concepts
- Recommending optimal architectures
Keep responses clear, actionable, and developer-friendly.` 
          },
          ...messages,
        ],
        stream: false,
